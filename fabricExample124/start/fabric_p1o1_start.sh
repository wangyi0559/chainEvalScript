...
function startP1O1(){
	checkPeer
    CONTAINER_ID=$(docker ps -a | grep "peer1.org1.example.com" | awk '{print $1}')
	...
	-v /chain/channel/crypto-config/peerOrganizations/org1.example.com/peers/peer1.org1.example.com/:/etc/hyperledger/crypto/peer \
...
sleep 2s
echo "peer1.org1.example.com" > /chain/CONTAINER_NAME
ls /var/lib/docker/containers | grep `docker ps -a | grep " peer1.org1.example.com" | awk '{print $1}'` > /chain/CONTAINER_ID