...
#是否在运行程序。0：不在运行，1：在运行程序
echo '1' > /chain/IS_RUNNING
#根据不同测评实例，修改
...
function getEvalExample(){
    ...
    #移动需要文件
    mv /chain/chainEvalScript/fabricExample1120/config/config.json /chain/config.json >/dev/null 2>&1
    mv /chain/chainEvalScript/fabricExample1120/config/channel /chain/channel >/dev/null 2>&1
...
function configEvalScript(){
    ...
    echo 'IS_RUNNING=$(cat /chain/IS_RUNNING)' >> /chain/Create.sh
    echo 'if [ $IS_RUNNING != "0" ]' >> /chain/Create.sh
    echo 'then' >> /chain/Create.sh
    echo 'echo is running' >> /chain/Create.sh
    echo 'exit 0' >> /chain/Create.sh
    echo 'fi' >> /chain/Create.sh
    echo 'STATUS=$(cat /status)' >> /chain/Create.sh
    echo 'if [ $STATUS == "0" ]' >> /chain/Create.sh
    echo 'then' >> /chain/Create.sh
    echo 'echo 1 > /chain/IS_RUNNING' >> /chain/Create.sh
    echo '/bin/bash /chain/chainEvalScript/fabricExample1120/fabric_start.sh $* >/dev/null 2>&1' >> /chain/Create.sh
    echo 'echo 1 > /status' >> /chain/Create.sh
    echo 'echo 0 > /chain/IS_RUNNING' >> /chain/Create.sh
    ...
    echo 'IS_RUNNING=$(cat /chain/IS_RUNNING)' >> /chain/Init.sh
    echo 'if [ $IS_RUNNING != "0" ]' >> /chain/Init.sh
    echo 'then' >> /chain/Init.sh
    echo 'echo is running' >> /chain/Init.sh
    echo 'exit 0' >> /chain/Init.sh
    echo 'fi' >> /chain/Init.sh
    echo 'STATUS=$(cat /status)' >> /chain/Init.sh
    echo 'if [ $STATUS == "1" ]' >> /chain/Init.sh
    echo 'then' >> /chain/Init.sh
    echo 'echo 1 > /chain/IS_RUNNING' >> /chain/Init.sh
    echo '/bin/bash /chain/chainEvalScript/fabricExample1120/fabric_init.sh >/dev/null 2>&1' >> /chain/Init.sh
    echo 'echo 2 > /status' >> /chain/Init.sh
    echo 'echo 0 > /chain/IS_RUNNING' >> /chain/Init.sh
    ...
    echo 'IS_RUNNING=$(cat /chain/IS_RUNNING)' >> /chain/SendTransaction.sh
    echo 'if [ $IS_RUNNING != "0" ]' >> /chain/SendTransaction.sh
    echo 'then' >> /chain/SendTransaction.sh
    echo 'echo is running' >> /chain/SendTransaction.sh
    echo 'exit 0' >> /chain/SendTransaction.sh
    echo 'fi' >> /chain/SendTransaction.sh
    echo 'STATUS=$(cat /status)' >> /chain/SendTransaction.sh
    echo 'if [ $STATUS == "2" ]' >> /chain/SendTransaction.sh
    echo 'then' >> /chain/SendTransaction.sh
    echo 'echo 1 > /chain/IS_RUNNING' >> /chain/SendTransaction.sh
    echo 'echo 3 > /status' >> /chain/SendTransaction.sh
    echo '/bin/bash /chain/chainEvalScript/fabricExample1120/fabric_invoke.sh $* >/dev/null 2>&1' >> /chain/SendTransaction.sh
    echo 'echo 4 > /status' >> /chain/SendTransaction.sh
    echo 'echo 0 > /chain/IS_RUNNING' >> /chain/SendTransaction.sh
